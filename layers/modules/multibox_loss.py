...
class MultiBoxLoss(nn.Module):
    ...
    def execute(self, predictions, targets):
        ...
        loc_t = jt.zeros((num, num_priors, 4))
        conf_t = jt.zeros((num, num_priors), dtype=jt.int64)
        
        for idx in range(num):
            truths = targets[idx][:, :-1]
            labels = targets[idx][:, -1].int()
            defaults = priors
        ...
        loc_t_val = loc_t[pos_idx].view(-1, 4)
        loss_l = nn.smooth_l1_loss(loc_p, loc_t_val, reduction='sum')
        ...
        loss_c = loss_c * (1 - pos.view(-1, 1).float())  # filter out pos boxes for now
        loss_c = loss_c.view(num, -1)
        _, loss_idx = loss_c.sort(1, descending=True)
        _, idx_rank = loss_idx.sort(1)
        num_pos = pos.int().sum(1, keepdims=True)
        num_neg = jt.clamp(self.negpos_ratio * num_pos, 
                          min_v=0, max_v=pos.shape[1] - 1)
        neg = idx_rank < num_neg.expand_as(idx_rank)

        # Confidence Loss Including Positive and Negative Examples
        pos_idx = pos.unsqueeze(2).expand_as(conf_data)
        neg_idx = neg.unsqueeze(2).expand_as(conf_data)
        mask = (pos_idx + neg_idx) > 0
        conf_p = conf_data[mask].view(-1, self.num_classes)
        targets_weighted = conf_t[(pos + neg) > 0]
        
        loss_c = nn.cross_entropy_loss(conf_p, targets_weighted, reduction='sum')

        # Sum of losses: L(x,c,l,g) = (Lconf(x, c) + αLloc(x,l,g)) / N
        N = num_pos.sum() if num_pos.sum().item() > 0 else num
        loss_l = loss_l / N
        loss_c = loss_c / N
